# Chrome Web Store listing — TCG Lens
...
```
TCG Lens puts live Pokémon card market values right on the listings you're already browsing.
...
## Package
Upload `tcglens.zip` (built from the repo — runtime files only). See README / build step.
```
